...
public class PublicBookingController {
    ...
    public ResponseEntity<?> getBlockedDates(
        ...
        try {
            ...
            // includePrices retiré — trop lent, provoque des 504. Les prix apparaissent via /estimate.
            ...
            // 2. Calendrier Beds24 : blackouts uniquement (déjà lancé en parallèle)
            try {
                ...
                for (Map<String, Object> room : rooms) {
                    ...
                    for (Object o : cal) {
                        ...
                        while (!cur.isAfter(end)) {
                            if (!cur.isBefore(fromDate) && !cur.isAfter(toDate)) blocked.add(cur.toString());
                            cur = cur.plusDays(1);
                        }
                    }
                }
            ...
            return ResponseEntity.ok(Map.of("blockedDates", blocked, "prices", Map.of()));
        } catch (Exception e) {
            return error(e);
        }
    }
    ...
    public ResponseEntity<?> createBooking(
        ...
        try {
            ...
            if (!saved.isEmpty()) {
                ...
                if (!hasId && fallbackPropId != null && fallbackArrival != null) {
                    Map<String, String> fp = new HashMap<>();
                    fp.put("propertyId",  fallbackPropId);
                    fp.put("arrivalFrom", fallbackArrival);
                    fp.put("arrivalTo",   fallbackArrival);
