...
### Gamification — Prestataires, hôtes, voyageurs

- **Principe directeur** : points/badges/niveaux sont **toujours calculés à la volée** depuis les tables existantes (même logique que `HousekeepingReportService.costSummary()`) — aucun grand livre à maintenir en synchro, aucune nouvelle entité sauf là où c'est strictement nécessaire. Conséquence assumée côté prestataires : les points peuvent redescendre si une tâche `DONE` est modifiée/supprimée après coup — comportement voulu, pas un bug.
- **Prestataires** (`HousekeeperRewardsService`, `GET /housekeeper/rewards`) : +10 pts par mission `DONE` sans incident, +2 pts par mission `DONE` avec incident correctement signalé. Niveaux Bronze/Argent/Or/Platine (seuils 0/100/300/700). Badges : "Premières missions" (10), "Habitué·e" (50), "Vétéran·e" (100), "Zéro incident" (20 dernières missions sans incident), "Rapporteur·se fiable" (10 rapports avec photos). **Classement** entre prestataires du même hôte (`HousekeepingTaskRepository.countDoneGroupedByHousekeeperForHost`) — requêtes strictement scalaires (`COUNT`/`GROUP BY`, jamais `List<HousekeepingTask>`) car `HousekeepingTask.housekeeper` est en `FetchType.EAGER` → un classement multi-prestataires en entités complètes déclencherait un SELECT N+1 par prestataire distinct. 3ᵉ onglet du portail (`housekeeper-rewards.component.ts`), à côté de Missions et Signalements.
- **Hôtes** (`HostRewardsService`, `GET /admin/rewards`, menu "Récompenses") : badges calculés en vérifiant l'existence de données déjà en base (Beds24 connecté, ≥1 logement, Stripe connecté, ≥1 code promo, répondeur auto actif, ≥1 prestataire, site public en ligne, ancienneté 1 mois/3 mois/1 an) — **pas** d'event-log dédié (`AnalyticsEvent` ne trace que LOGIN/PAGE_VIEW/CLICK, inutilisable comme déclencheur de badge). Points = 50 par badge débloqué. Niveaux Débutant/Confirmé/Expert/Ambassadeur (seuils 0/100/250/400). **Pas de classement entre hôtes** (décision produit : comparer des clients payants entre eux pose un risque de positionnement B2B).
- **Voyageurs** (`GuestRewardService`, site de réservation public) : aucune donnée de réservation n'existe localement en mode Beds24 (`Booking`/`Guest` ne sont pas des entités JPA) — `GuestBookingLog` (nouvelle table, `user`, `beds24PropertyId`, `guestEmail` indexé en minuscules, `beds24BookingId`, `createdAt`) trace chaque réservation directe créée via `PublicBookingController.createBooking()`, pour compter les séjours d'un même email **sans appel Beds24 supplémentaire** (l'API a un quota de crédits documenté plus haut). Au 3ᵉ/5ᵉ/10ᵉ séjour détecté, génère automatiquement un `PromoCode` à usage unique (`restrictedGuestEmail` non null → désactivé après sa première utilisation, dans le même bloc que l'incrément `usageCount` existant) et l'affiche sur l'écran de confirmation (`booking-site-confirmation.component.ts`), transmis via `router.navigate(..., { state: { guestReward } })` — même mécanisme `history.state` que `payOnline()`. **Limites assumées** : ne compte pas rétroactivement les réservations antérieures à cette fonctionnalité ; restriction "soft" (usage unique, pas de vérification d'identité à l'estimation puisque l'email n'est connu qu'à la soumission finale) ; perdu si la page de confirmation est rafraîchie (même limite déjà acceptée pour le flux de paiement) ; non disponible en mode iCal (pas de site public de réservation dans ce mode).

### Email de confirmation automatique — Réservations directes (IA)

- **`EmailService`** (`backend/src/main/java/com/flowlyrent/service/EmailService.java`) : service SMTP générique préexistant (`JavaMailSender`), jusqu'ici non câblé à aucun déclencheur. `isConfigured()` vérifie **username ET password** (pas seulement l'adresse) — évite des tentatives d'envoi systématiquement en échec (et des appels Groq gaspillés) sur un environnement sans mot de passe SMTP configuré.
- **Config** : `spring.mail.*` dans `application.yml`, `MAIL_HOST`/`MAIL_PORT` (défauts `smtp.gmail.com:587`), `MAIL_USERNAME` (défaut non-sensible `contact@flowlyrent.com`), `MAIL_PASSWORD` (secret, **mot de passe d'application Gmail**, pas le mot de passe du compte — à générer dans les paramètres de sécurité Google si compte Gmail/Workspace).
- **`BookingConfirmationEmailService.sendConfirmation(...)`** : déclenché automatiquement dans `PublicBookingController.createBooking()` après une réservation directe réussie (site public, mode Beds24 uniquement — même périmètre que la fidélité voyageurs ci-dessus), `@Async`. Contenu de l'email **rédigé par l'IA** (Groq, même client HTTP statique partagé que `AutoResponderService`/`MessageAssistService`) : logement, dates, ton chaleureux, ne mentionne jamais le code d'accès (transmis séparément avant l'arrivée). Si une récompense fidélité vient d'être débloquée (`guestReward`), l'email la mentionne aussi.
- **Repli sans IA** : si `GROQ_API_KEY` absent ou l'appel échoue, `fallbackBody()` génère un email HTML simple mais complet (mêmes informations, sans reformulation IA) — l'échec de Groq ne bloque jamais l'envoi de l'email.
- **Échec silencieux volontaire** : comme pour le reste du flux `createBooking()`, une erreur d'envoi (SMTP mal configuré, Groq indisponible) est loguée en warning mais ne fait jamais échouer la création de la réservation.
