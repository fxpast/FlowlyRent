...
- **`PromoCode`** (table `promo_codes`, unique par `(user_id, code)`) : `beds24PropertyId`, `code`, `discountPercent` (1-100), `active`, `usageCount`. Géré dans **Logements → Paramètres du logement** (section "Codes promo" sous la Tarification directe, `properties.component.ts`) — CRUD via `AdminPromoCodeController` (`GET/POST /admin/promo-codes`, `PUT/DELETE /admin/promo-codes/{id}`).
