...
const STATUS_META: Record<string, { color: string; icon: string }> = {
  PENDING:     { color: '#e65100', icon: 'schedule' },
  IN_PROGRESS: { color: '#1565c0', icon: 'play_circle' },
  DONE:        { color: '#2e7d32', icon: 'check_circle' },
  SKIPPED:     { color: '#757575', icon: 'cancel' }
};
...
@Component({
  ...
  imports: [
    ...
    MatFormFieldModule, MatInputModule, MatTabsModule, TranslateModule
  ],
  template: `
    <mat-tab-group animationDuration="200ms">

      <!-- ── Onglet Missions ── -->
      <mat-tab [label]="'housekeeper.tab_missions' | translate">
        <div class="tab-content">
          @if (loading()) {
            ...
            <div class="empty-state">
              ...
              <p>{{ 'housekeeper.no_tasks' | translate }}</p>
            </div>
          } @else {
                  ...
                  <mat-card class="task-card" [class.done]="task.status === 'DONE'" [class.in-progress]="task.status === 'IN_PROGRESS'">
                    ...
                    <div class="task-head">
                      <div class="task-date-type">
                        ...
                        <div class="task-date">{{ formatTaskDate(task.scheduledDate) }}</div>
                      </div>
                    ...
                    @if (task.extraHours != null) {
                      <div class="task-hours">
                        ...
                        {{ 'housekeeper.duration_label' | translate }} <strong>{{ task.extraHours }} h</strong>
                      </div>
                    }
                      ...
                      <div class="task-actions">
                        @if (task.status === 'PENDING') {
                          <button mat-flat-button color="primary" (click)="setStatus(task, 'IN_PROGRESS')">
                            <mat-icon>play_arrow</mat-icon> {{ 'housekeeper.start' | translate }}
                          </button>
                        }
                        @if (task.status === 'IN_PROGRESS') {
                          <button mat-flat-button color="accent" class="done-btn" (click)="setStatus(task, 'DONE')">
                            <mat-icon>check</mat-icon> {{ 'housekeeper.complete' | translate }}
                          </button>
                        }
                      </div>
                    }
                    ...
                    @if (task.status === 'DONE' && task.completedAt) {
                      <div class="completed-line">
                        ...
                        {{ 'housekeeper.completed_on' | translate }} {{ task.completedAt | date:'dd/MM HH:mm' }}
                      </div>
                    }
                      ...
                      <div class="existing-report" [class.incident]="task.hasIncident">
                        ...
                        <div>
                          ...
                          @if (task.hasIncident && task.incidentDescription) {
                            <div class="incident-text">{{ 'housekeeper.incident_prefix' | translate }} {{ task.incidentDescription }}</div>
                          }
                        </div>
                      </div>
                    }
                    ...
                    <div class="report-toggle">
                      <button mat-stroked-button (click)="toggleReport(task)">
                        ...
                        {{ openReport() === task.id ? ('common.close' | translate) : ('housekeeper.report_photos' | translate) }}
                      </button>
                    </div>
                    ...
                    @if (openReport() === task.id) {
                      <div class="report-form">
                        <mat-form-field style="width:100%">
                          <mat-label>{{ 'housekeeper.comment' | translate }}</mat-label>
                          <textarea matInput rows="3" [(ngModel)]="draft.comment"></textarea>
                        </mat-form-field>

                        <div class="incident-toggle">
                          <mat-slide-toggle color="warn" [(ngModel)]="draft.hasIncident">
                            {{ 'housekeeper.report_incident' | translate }}
                          </mat-slide-toggle>
                        </div>
                        ...
                        <div class="photo-section">
                          <div class="photo-section-title">{{ 'housekeeper.photos' | translate }}</div>
                          @if (uploadingPhoto()) {
                            ...
                            <div class="photo-type-row">
                              <button mat-stroked-button type="button" (click)="triggerPhoto('BEFORE', photoInput)">
                                <mat-icon>photo_camera</mat-icon> {{ 'housekeeper.photo_before' | translate }}
                              </button>
                              <button mat-stroked-button type="button" (click)="triggerPhoto('AFTER', photoInput)">
                                <mat-icon>photo_camera</mat-icon> {{ 'housekeeper.photo_after' | translate }}
                              </button>
                              <button mat-stroked-button color="warn" type="button" (click)="triggerPhoto('INCIDENT', photoInput)">
                                <mat-icon>warning</mat-icon> {{ 'housekeeper.photo_incident' | translate }}
                              </button>
                            </div>
                        ...
                        <div class="report-actions">
                          ...
                          <button mat-button (click)="closeReport()">{{ 'common.cancel' | translate }}</button>
                        </div>
                      </div>
                    }
      ...
      <!-- ── Onglet Arrivées ── -->
      <mat-tab [label]="'housekeeper.tab_arrivals' | translate">
        <div class="tab-content">
          @if (loadingArrivals()) {
            <div class="center"><mat-spinner diameter="40"/></div>
            ...
            <div class="empty-state">
              ...
              <p>{{ 'housekeeper.no_arrivals' | translate }}</p>
            </div>
          } @else {
            @for (b of arrivals(); track b['id']) {
              <mat-card class="booking-card">
                <div class="booking-head">
                  <mat-icon>flight_land</mat-icon>
                  <div class="booking-guest">{{ b['guestFirstName'] || b['firstName'] }} {{ b['guestLastName'] || b['lastName'] }}</div>
                </div>
                <div class="booking-prop">
                  <mat-icon>home</mat-icon>
                  {{ b['propName'] ?? b['propId'] }}
                </div>
                <div class="booking-dates">
                  <div class="booking-date-row">
                    ...
                    <span>{{ 'housekeeper.checkin_date' | translate }} <strong>{{ b['arrival'] | date:'dd/MM/yyyy' }}</strong></span>
                  </div>
                  <div class="booking-date-row">
                    <mat-icon>logout</mat-icon>
                    <span>{{ 'housekeeper.checkout_date' | translate }} <strong>{{ b['departure'] | date:'dd/MM/yyyy' }}</strong></span>
                  </div>
                </div>
                @if (b['numAdult'] || b['numChild']) {
                  <div class="booking-guests-count">
                    <mat-icon>group</mat-icon>
                    {{ (b['numAdult'] || 0) + (b['numChild'] || 0) }} pers.
                  </div>
                }
              </mat-card>
            }
      ...
      <!-- ── Onglet Départs ── -->
      <mat-tab [label]="'housekeeper.tab_departures' | translate">
        <div class="tab-content">
          @if (loadingDepartures()) {
            ...
            <div class="empty-state">
              ...
              <p>{{ 'housekeeper.no_departures' | translate }}</p>
            </div>
          } @else {
  ...
  styles: [`
    ...
    .booking-card { padding: 14px; margin-bottom: 12px; border-radius: 12px !important; }
    .booking-head { display: flex; align-items: center; gap: 8px; margin-bottom: 8px; }
    .booking-head mat-icon { color: #1565c0; }
    .booking-guest { font-size: 16px; font-weight: 700; }
    .booking-prop { display: flex; align-items: center; gap: 6px; font-size: 14px; color: #444; margin-bottom: 8px; }
    .booking-prop mat-icon { font-size: 18px; width: 18px; height: 18px; color: #888; }
    .booking-dates { display: flex; flex-direction: column; gap: 4px; margin-bottom: 6px; }
    .booking-date-row { display: flex; align-items: center; gap: 6px; font-size: 13px; color: #555; }
    .booking-date-row mat-icon { font-size: 16px; width: 16px; height: 16px; color: #999; }
...
export class HousekeeperTasksComponent implements OnInit {
  ...
  loadingArrivals = signal(true);
  loadingDepartures = signal(true);
  ...
  typeLabel(type: string)   { return this.t.instant('task_type.' + type); }
  statusLabel(s: string) { return this.t.instant('status.' + s.toLowerCase()); }
  ...
  saveReport(task: TaskWithPhotos): void {
    ...
    this.svc.saveReport(task.id, {
      ...
      next: updated => {
        ...
        this.snack.open(this.t.instant('housekeeper.report_saved'), '', { duration: 2000 });
        this.closeReport();
      },
  ...
  private uploadFilesSequentially(files: File[], task: TaskWithPhotos, index: number): void {
    ...
    this.zone.run(() => {
      ...
      this.uploadProgress.set(files.length > 1 ? this.t.instant('housekeeper.uploading_n', { current: index + 1, total: files.length }) : this.t.instant('housekeeper.uploading'));
    });
    this.compressImage(files[index]).then(base64 => {
      ...
      fetch(`${this.svc.baseUrl}/tasks/${task.id}/photos`, {
        ...
        this.zone.run(() => {
          ...
          const msg = status === '413' ? this.t.instant('housekeeper.photo_heavy') : `${this.t.instant('housekeeper.upload_error')} (${status})`;
          this.snack.open(msg, this.t.instant('common.close'), { duration: 5000 });
        });
      });
    }).catch(() => {
      this.zone.run(() => {
        this.uploadingPhoto.set(false);
        this.snack.open(this.t.instant('housekeeper.photo_error'), this.t.instant('common.close'), { duration: 3000 });
      });
    });
  }
